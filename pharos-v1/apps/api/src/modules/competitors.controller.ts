...
import { CompetitorCreateSchema } from "@pharos/shared";
...
export class CompetitorsController {
  ...
  create(@CurrentUser() user: { tenantId: string }, @Body(new ZodValidationPipe(CompetitorCreateSchema)) body: { name: string }) {
    return prisma.competitor.create({ data: { ...body, tenantId: user.tenantId } });
  }
  ...
  async update(
    ...
    @Param("id") id: string,
    @Body(new ZodValidationPipe(CompetitorCreateSchema.partial())) body: Partial<{ name: string }>,
  ) {
    const found = await prisma.competitor.findFirst({ where: { id, tenantId: user.tenantId } });
    assertTenantOwned(found);
    if (Object.keys(body).length === 0) throw new BadRequestException("No update payload");
    return prisma.competitor.update({ where: { id }, data: body });
  }
