...
export class SalesController {
  ...
  async bulk(
    ...
    @Body(new ZodValidationPipe(SaleBulkSchema)) body: {
      rows: { skuId: string; dealerId: string; soldPrice: number; qty: number; soldAt: Date; orderRef: string }[];
    },
  ) {
    const skuIds = [...new Set(body.rows.map((r) => r.skuId))];
    const dealerIds = [...new Set(body.rows.map((r) => r.dealerId))];
    ...
    if (skuCount !== skuIds.length || dealerCount !== dealerIds.length) {
      throw new BadRequestException("Invalid skuId or dealerId for tenant");
    }
